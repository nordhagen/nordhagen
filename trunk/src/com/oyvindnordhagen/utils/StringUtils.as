...
{
	import com.oyvindnordhagen.framework.utils.AILogger;
	...
	public class StringUtils 
	{
		...
		public static function removeXmlIndents($string:String,
												$removeTabs:Boolean = true,
												$removeReturns:Boolean = true,
												$removeExtraSpaces:Boolean = true):String 
		{
		...
		public static function formatTime(seconds:uint, dhms:Array = null):String 
		{
			var d:Number = Math.floor( seconds / 86400 ); // 86400 s in 24 hours
			var h:Number = Math.floor( seconds / 3600 ); // 3600 s in one hour
			var m:Number = s % 60;
		 ...
		 * @return String representation of the fractional value as percent with % symbol. 
		 */
		public static function currency(value:String, startSymbol:String = null, endSymbol:String = null,
		thousandSeparator:String = ".", decimalPoint:String = ",", numDecimals:uint = 0):String
		{
			thousandSeparator = (thousandSeparator) ? thousandSeparator : "."; // Default if set to null to reach next argument
			decimalPoint = (decimalPoint) ? decimalPoint : ","; // Default if set to null to reach next argument
			value = value.replace( new RegExp( "/\\" + thousandSeparator + "/", "g" ), "" ); // Remove thousand separators if any
			startSymbol = (startSymbol) ? startSymbol : ""; // Deafult to empty string if null
			endSymbol = (endSymbol) ? endSymbol : ""; // Deafult to empty string if null
			if (value.indexOf( decimalPoint ) != -1 ) numDecimals = value.length - value.indexOf( decimalPoint ); // Use position of existing decimal point if any
			var decimals:String = (numDecimals > 0) ? "," + value.substr( -numDecimals ) : ""; // Seperate decimals if any
			var nonDecimals:String = value.substr( 0, value.length - numDecimals ); // Seperate integers
			return startSymbol + addSeparator( nonDecimals, ".", 3 ) + decimals + endSymbol; // Return formatted
		}

		public static function addSeparator(value:String, separator:String = ".", charsBetween:uint = 3):String
		{
		...
		public static function parseLinks($string:String, $target:String = "_self"):String 
		{
		...
		{
			_numIterations++;
			AILogger.header( "get Overflow Text" );
			...
			textField.textHeight; // Forces internal measuring of text bounds. **PLAYER BUG**
			var ret:String;
			...
			else 
			{
				...
				var firstOverflowLine:uint = numLines - maxScroll; // bottomScrollV is 1-based
				var firstOverflowChar:uint = textField.getLineOffset( firstOverflowLine ); // getLineOffset is 0-based
				AILogger.log( "textField has " + textField.text.length + " characters, " + textField.numLines + " lines and a height of " + textField.height );
				AILogger.log( "firstOverflowLine: " + firstOverflowLine + " (\"" + textField.getLineText( firstOverflowLine ) + "\")" );
				AILogger.log( "Looking for break points around index " + firstOverflowChar );
				...
				else 
				{
					...
					AILogger.log( "getPreceedingBreakPointIndex found split possibility at char " + splitIndex + " (" + text.substr( splitIndex, 5 ) + ")" );
				}
			...
			AILogger.log( "splitIndex: " + splitIndex );
			AILogger.log( "Length of overflow text: " + ret.length );
			AILogger.log( "Lenght of text left in source field: " + textField.text.length );
			AILogger.log( "Completed iteration number " + _numIterations );
			return ret;
		}
		...
		public static function getTagAndContents(text:String, lookAroundIndex:uint, startTag:String = null, endTag:String = null):Array 
		{
			...
			AILogger.log( "Start tag found at index " + finalStartIndex + " after " + numMatches + " iterations" );
