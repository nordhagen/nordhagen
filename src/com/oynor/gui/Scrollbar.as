...
package com.oynor.gui {
	...
	import com.oynor.framework.interfaces.IDestroyable;
	...
	[Event(name="animationStarted",type="com.oynor.events.AnimationEvent")]
	[Event(name="animationComplete",type="com.oynor.events.AnimationEvent")]
	public class Scrollbar extends Sprite implements IDestroyable {
		...
		// The foreground rectangle visually representing the scrollbar
		protected var _handleDrawVO:DrawVO;
		protected var _handleDim:Rectangle;
		protected var _innerPadding:Number;
		protected var _mask:*;
		// The mask over scrollingClip, effectively becoming the viewport
		protected var _maskLastBounds:Rectangle = new Rectangle();
		protected var _maskShowFactor:Number;
		// How much of the scrollingClip the viewPort is able to show
		// accommodate any y position not 0
		protected var _maxScroll:uint;
		// The negative offset in pixels that represents having scrolled the
		protected var _target:DisplayObject;
		// The masked movie clip that is controlled by the scollbar
		protected var _targetLastBounds:Rectangle = new Rectangle();
		protected var _targetYOrigin:uint;
		// The pixel amount to add to scrollingClip position to
		protected var _track:Sprite;
		// The background rectangle visually representing the travel area for the scrollbar
		protected var _trackDrawVO:DrawVO;
		protected var _trackDim:Rectangle;
		protected var _travelHeight:uint;
		// The height of the scrollbar minus the height of the scroller
		 ...
		 */
		public function Scrollbar ( $target:DisplayObject, $mask:*, $trackDrawVO:DrawVO = null, $handleDrawVO:DrawVO = null, $trackWidth:uint = 12, $handleWidth:uint = 8 ) {
		...
		protected function _draw ():void {
			...
			_travelHeight = th - hh - _innerPadding * 2;
			// The max X-position for the scroller
		...
		protected function _updateSize ():void {
			...
			_maskShowFactor = _mask.height / _target.height;
			// Fraction for the amount of content visible in viewPort
			_maxScroll = _target.height - _mask.height;
			// Maximum negative offset for scrollingClip (= bottom of content)
		}
	}
}